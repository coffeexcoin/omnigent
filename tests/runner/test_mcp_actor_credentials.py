...
async def test_http_credentials_resolve_per_request_and_pool_by_actor_generation(
    ...
    try:
        ...
        await manager.call_tool(spec, "github__whoami", {}, actor=alice, session_id="session-1")
        await manager.call_tool(spec, "github__whoami", {}, actor=bob, session_id="session-1")
        ...
        assert created[2].config.headers["Authorization"] == ("Bearer token-alice@example.com-2")
...
async def test_stdio_processes_are_scoped_by_session_actor_and_generation(
    ...
    try:
        await manager.call_tool(spec, "github__whoami", {}, actor=alice, session_id="session-1")
        await manager.call_tool(spec, "github__whoami", {}, actor=alice, session_id="session-2")
        await manager.call_tool(spec, "github__whoami", {}, actor=bob, session_id="session-1")
...
async def test_actor_credential_fails_closed_without_active_actor(
    ...
    created = _patch_connections(monkeypatch)
    manager = RunnerMcpManager(credential_resolver=_CredentialResolver(transport="http"))
    spec = _spec(
        MCPServerConfig(
            name="github",
            transport="http",
...
async def test_actor_stdio_credential_fails_closed_without_active_session(
    ...
    created = _patch_connections(monkeypatch)
    manager = RunnerMcpManager(credential_resolver=_CredentialResolver(transport="stdio"))
    spec = _spec(
        MCPServerConfig(
            name="github",
            transport="stdio",
